...
class Enchere extends \Core\Model
{
    ...
    {
        $db = static::getDB();
        $stmt = $db->prepare("SELECT enchere.id, titre, commentaire, debut, fin, prix_plancher, offre_actuel, offre_actuel_membre, quantite_mise, a_coup_de_coeur_lord, est_enligne FROM enchere left join timbre on enchere.id=id_enchere where id_utilisateur = ?");
