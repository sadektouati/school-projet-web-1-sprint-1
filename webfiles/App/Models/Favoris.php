...
class Favoris extends \Core\Model
{
    ...
    {
        $db = static::getDB();
        $stmt = $db->prepare("SELECT enchere.id, titre, commentaire, debut, fin, prix_plancher, offre_actuel, offre_actuel_membre, quantite_mise, a_coup_de_coeur_lord, est_enligne FROM enchere join favoris on enchere.id=id_enchere where favoris.id_utilisateur = ?");
